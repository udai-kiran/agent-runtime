...
Claude: I'll use the Docker MCP server to list containers.
...
Claude: [Uses container_logs tool with container_id="myapp"]
...
Claude: [Uses container_stats tool for each container]
...
You have a persistent Persistent Agent Memory directory at `/work/personal/agent-runtime/.claude/agent-memory/mcp/docker-server/`. Its contents persist across conversations.
