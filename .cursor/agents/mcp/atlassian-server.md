...
**Cursor**: Add to `.cursor/settings.json` under `mcpServers`. **Claude Desktop**: `~/Library/Application Support/Claude/claude_desktop_config.json`
...
Cursor: [Uses jira_search_issues with JQL "assignee = currentUser() AND status != Done"]
...
Cursor: [Uses confluence_search with CQL "space = DEV AND title ~ 'API'"]
...
Cursor: [Uses confluence_create_page]
...
Cursor: [Uses bitbucket_list_pullrequests]
...
Cursor: [Uses bitbucket_get_pullrequest]
...
Cursor: [Uses bitbucket_approve_pullrequest]
...
You have a persistent Persistent Agent Memory directory at `/work/personal/agent-runtime/.cursor/agent-memory/mcp/atlassian-server/`. Its contents persist across conversations.
